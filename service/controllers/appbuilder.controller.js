...
module.exports.getBusinessEntities = async (req, res) => {
    try {
      let data = db.BusinessEntitySchema.findAll({
        where: {
          _status: "active",
        },
      });
...
module.exports.getIndividualEntityData = async (req, res) => {
    ...
    try {
      ...
      if (!data) {
        res.status(204).json({
          message: "Entity[" + entity + "] data not found",
        });
        return;
      }
...
module.exports.noAuthGetAllEntityData = async (req, res) => {
    ...
    try {
      ...
      modelNames
        ...
        .forEach(async (model) => {
          ...
          let _tempForm = await db.FormSchemas.findOne({
            where: { formID: model },
          });
          if (_tempForm) {
    ...
    try {
      ...
      paginate(db.FormSchemas, [], baseQuery, pageQuery)
        ...
        .catch((err) => {
          ...
          res
            .status(500)
            .json({ message: "Error to fetch form schemas", error: err });
        });
    } catch (error) {
      console.error(error);
      res.status(500).json({
  ...
  module.exports.getSpecificFormSchema = async (req,res) => {
    try {
      ...
      let formSchema = await db.FormSchemas.findOne({
        where: {
          formID: formID,
        },
      });
  ...
  module.exports.createRxFormSchema = async (req,res) =>  {
    try {
      ...
      // update model
      var result = await db.FormSchemas.create({
        ...req.body,
        createdBy: req.user.userId,
        updatedBy: req.user.userId,
      });
      ...
      res
        .status(500)
        .json({ message: "Error to create form schema", error: error });
    }
  }
  module.exports.updateRxFormSchema = async (req,res) => {
    try {
      ...
      // update model
      var result = await db.FormSchemas.update(
        {
          ...req.body,
          updatedBy: req.user.userId,
        },
      ...
      res
        .status(500)
        .json({ message: "Error to update form schema", error: error });
    }
  }
  module.exports.deleteRxFormSchema = async (req,res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error to delete form schema", error: error });
    }
  }